...
def parse_day_file(file_path):
    ...
    try:
        ...
        code = stock_filename[2:] if stock_filename.startswith('sh') or stock_filename.startswith('sz') else stock_filename
        ...
        date_int, open_price_int, high_price_int, low_price_int, close_price_int, \
        amount, volume, prev_close_int = struct.unpack('<iiiiifii', latest_record)
        ...
        # 获取股票名称和行业
        stock_info = STOCK_NAME_INDUSTRY_MAP.get(code, {'name': f'{code}.{market}', 'industry': '未知行业'})
...
def main():
    ...
    # 解析lday目录下的所有.day文件
    # 先处理沪市A股
    sh_lday_dir = os.path.join(os.path.dirname(os.path.abspath(__file__)), '..', 'data', 'original', 'lday', 'sh', 'lday')
    # 再处理深市A股
    sz_lday_dir = os.path.join(os.path.dirname(os.path.abspath(__file__)), '..', 'data', 'original', 'lday', 'sz', 'lday')
    output_file = os.path.join(os.path.dirname(os.path.abspath(__file__)), '..', 'data', 'processed', 'stock_data.json')
    
    # 检查并处理沪市数据
    if os.path.exists(sh_lday_dir):
        print("开始解析沪市A股日线数据...")
        parse_all_day_files(sh_lday_dir, output_file)
    else:
        print(f"警告: 找不到沪市A股日线数据目录 {sh_lday_dir}")
        # 尝试直接在lday目录下查找.day文件
        direct_lday_dir = os.path.join(os.getcwd(), 'lday')
        if os.path.exists(direct_lday_dir):
            print("尝试直接在lday目录下解析.day文件...")
            parse_all_day_files(direct_lday_dir, output_file)
        else:
            print(f"错误: 找不到lday目录 {direct_lday_dir}")
    
    # 检查并处理深市数据（如果沪市数据解析成功）
    if os.path.exists(sz_lday_dir) and os.path.exists(output_file):
        print("\n开始解析深市A股日线数据...")
        parse_all_day_files(sz_lday_dir, output_file)
